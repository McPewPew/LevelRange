...
DEFAULT_LEVELRANGE_SHOWFISHING	= true;
...
LEVELRANGE_FISHING = {
    [LEVELRANGE_ELWYNN]        	= { 1, 10, lTYPE_ALLIANCE},	
        ...
        local _, faction = UnitFactionGroup("player");
        local min = LEVELRANGE_FISHING[zoneName][1];
        local max = LEVELRANGE_FISHING[zoneName][2];
        local side = LEVELRANGE_FISHING[zoneName][3];
        ...
        fishinglevels = string.format(LEVELRANGE_FISHING, min, max);
    end
    ...
    if fishinglevels then
        LevelRangeTooltip:AddLine(fishinglevels, levelscol.r, levelscol.g, levelscol.b);
    end
